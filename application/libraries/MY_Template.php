...
class MY_Template extends CI_Template {
   ...
   {
      if (file_exists(VIEWPATH .'/'. $filename) or file_exists(VIEWPATH .'/'. $filename . EXT))
      {
   ...
   function initialize($props)
   {
      ...
      if (isset($props['template']) 
         && (file_exists(VIEWPATH.'/'. $props['template']) or file_exists(VIEWPATH .'/'. $props['template'] . EXT)))
      {
      ...
      else 
      {
         ...
         show_error('Either you have not provided a master template or the one provided does not exist in <strong>'. VIEWPATH .'</strong>. Remember to include the extension if other than ".php"');
      }
   ...
   function write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
      ...
      if (count($args) > 1)
      {
         ...
         {
            if (file_exists(VIEWPATH .'/'. $suggestion . EXT) or file_exists(VIEWPATH .'/'. $suggestion))
            {
   ...
   function front_write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
      ...
      $content = $this->CI->load->view(THEME_LOCATION.$view, $data, TRUE);
      $this->write($region, $content, $overwrite);
